...
typedef struct process{
    int id,At,Ct,Wt,Bt,Rt,pr,remaining_time;
    bool flag=false;
};
void arrange_as_PQS(process arr[],int n){
    int x=0,i,j;
    for(i=0;i<n-1;i++){
            x=i;
        for(j=i+1;j<n;j++){
            if(arr[j].At < arr[x].At) x=j;
            else if(arr[j].At==arr[x].At){
                if(arr[j].pr <arr[x].pr)
                    x=j;
            }
        }
        swap(arr[i],arr[x]);
...
void processes_after_PQS(process arr[],int n){

    cout<<"Pid\tArrival time\tBurst Time\tPriority\tWaiting Time\tResponse Time\tCompletion Time"<<endl;
    for(int i=0;i<n;i++)
    	cout<<arr[i].id<<"\t"<<arr[i].At<<"\t\t"<<arr[i].Bt<<"\t\t"<<arr[i].pr<<"\t\t"<<arr[i].Wt<<"\t\t"<<arr[i].Rt<<"\t\t\t"<<arr[i].Ct<<endl;
...
int main(){
    ...
    arrange_as_PQS(arr,n);
    calculate_RT_WT_CT(arr,n);
    cout<<"After Arranging \n";
    processes_after_PQS(arr,n);
    int a=0,b=0,c=0;
    for(int i=0;i<n;i++){
        a+=arr[i].Wt;
        b+=arr[i].Rt;
